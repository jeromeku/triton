...
def make_algo_decls(
    name: str, metas: Sequence[KernelLinkerMeta], generator=SignatureGenerator
) -> str:
    """Generate declarations of kernels with meta-parameter and constant values"""
    return f"""
CUresult {name}(CUstream stream, {generator.gen_signature_with_full_args(metas[-1])});
void load_{name}();
void unload_{name}();
    """


def make_global_decl(meta: KernelLinkerMeta, generator=SignatureGenerator) -> str:
    """Generate declarations of kernels with meta-parameter and constant values"""
    return f"""
CUresult {meta.orig_kernel_name}_default(CUstream stream, {generator.gen_signature_with_full_args(meta)});
CUresult {meta.orig_kernel_name}(CUstream stream, {generator.gen_signature_with_full_args(meta)}, int algo_id);
